...
struct Options {
	...
	static ParseError unexpected_arg(std::string_view const arg) { return ParseError{fmt::format("unexpected argument: '{}'", arg)}; }
	static ParseError unrecognized_opt(std::string_view const opt) { return ParseError{fmt::format("unrecognized option: '{}'", opt)}; }

	static std::string buildUsage(std::string_view const appName) { return fmt::format("usage: {} [-q|--quiet] [-s|--simulate] [path]", appName); }
...
struct App {
	...
	[[nodiscard]] static bool available() {
		// NOLINTNEXTLINE
		return std::system(fmt::format("{} --version > {}", command, null_v).c_str()) == 0;
	}

	static bool format(std::string_view const path) {
		if (!fs::is_regular_file(path)) { return false; }
		// NOLINTNEXTLINE
		return std::system(fmt::format("{} -i {}", command, path).c_str()) == 0;
	}
	...
	void run(Options const& options) const {
		if (!available()) { throw std::runtime_error{fmt::format("{} not available", command)}; }

		if (options.simulate && !options.quiet) { std::cout << "-- simulate enabled, source files will not be modified\n"; }

		auto count{0};
		if (!options.quiet) { std::cout << fmt::format("-- formatting source files in '{}' :\n\n", options.rootDir); }
		for (auto const& itr : fs::recursive_directory_iterator{options.rootDir}) {
			if (auto const& path = itr.path(); shouldFormat(path)) {
				...
				if (!options.quiet) { std::cout << fmt::format("{}\n", pathString); }
				if (!options.simulate) {
		...
		std::cout << fmt::format(" == {} source files formatted\n", count);
	}
};
} // namespace
...
int main(int argc, char** argv) {
	...
	try {
		...
		if (!fs::is_directory(options.rootDir)) {
			std::cerr << fmt::format("failed to resolve root directory '{}'\n", options.rootDir);
			return EXIT_FAILURE;
		}
